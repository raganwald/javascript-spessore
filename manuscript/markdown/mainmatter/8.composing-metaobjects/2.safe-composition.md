...
Bzzzzt! `tracy.songs()` should be an empty list, not `undefined`. The problem is that `HasAwards.initialize` overwrote `SingsSongs.initialize`, so we never executed `this._songs = [];`.
