...
We can promote this idea to composing metaobjects. First, we generalize `simpleProtocol` to handle an number of functions and the degenerate case of one function. We can then write `composeMetaobjects` to apply this to encapsulated methods:
