## Structs and Value Objects {#structs}
...
This looks very handy. For example, if we are fecthing rows of data in JSON from a server, structural equivalence is the only way to compare two rows for "equality." But there is a flaw. Note:
