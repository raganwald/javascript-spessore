...
Our `Person` object is a mixin, it provides some functionality to be mixed into an object with a function like `extend`. Using mixins does not require copying entire functions around, each object gets references to the functions in the mixin.

Things get even better: You can use more than one mixin with the same object:
...
We say that there is a *many-to-many* relationship between objects and mixins.
